...
void vts_pcm_dump_check_n_set(struct device *dev, int id)
{
	struct vts_data *data = dev_get_drvdata(dev);
	u32 values[3];

	if (vts_pcm_dump_get_file_started(id) == 0 || data->vts_ready == 0)
		return;

	switch (id) {
	case 2:  /* vts_internal node */
		values[0] = 0x00004000;
		values[1] = 0x1;
		values[2] = 0;
		dev_info(dev, "VTS Internal dump Start\n");
		vts_start_ipc_transaction(dev, data, VTS_IRQ_AP_COMMAND, &values, 0, 2);
		break;
	default:
		dev_info(dev, "%s: invalid id(%d)\n", __func__, id);
		break;
	}
}

void vts_pcm_dump_check_n_clr(struct device *dev, int id)
{
	struct vts_data *data = dev_get_drvdata(dev);
	u32 values[3];
	...
	case 2:  /* vts_internal node */
		...
		values[1] = 0x0;
		values[2] = 0;
		dev_info(dev, "VTS Internal dump Stop\n");
		vts_start_ipc_transaction(dev, data, VTS_IRQ_AP_COMMAND, &values, 0, 2);
		break;
	default:
		dev_info(dev, "%s: invalid id(%d)\n", __func__, id);
...
static int vts_pcm_dump_file_open(struct inode *i, struct file *f)
{
	...
	vts_pcm_dump_check_n_set(dev, info->id);
...
static int vts_pcm_dump_file_release(struct inode *i, struct file *f)
{
	...
	vts_pcm_dump_check_n_clr(dev, info->id);
