...
static void s2mpu16_temp_irq(struct s2mpu16_dev *s2mpu16, int degree)
{
	int temp = 0;
...
static void s2mpu16_buck_ocp_irq(struct s2mpu16_dev *s2mpu16, int buck)
{
...
static void s2mpu16_buck_oi_irq(struct s2mpu16_dev *s2mpu16, int buck)
{
...
{
	int ret;
	/* Unmask IRQM, Enable TX_IRQ interrupt */
	ret = s2mpu16_write_reg(s2mpu16->i2c, S2MPU16_COMMON_TX_MASK, 0x86);
	if (ret)
		goto err;
