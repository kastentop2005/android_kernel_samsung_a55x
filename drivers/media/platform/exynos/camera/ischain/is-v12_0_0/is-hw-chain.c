...
static const struct is_subdev_ops *(*subdev_ops[GROUP_SLOT_MAX])(void) = {
	...
	[GROUP_SLOT_SHRP] = pablo_get_is_subdev_shrp_ops,
...
static const int ioresource_to_hw_id[IORESOURCE_MAX] = {
	...
	[IORESOURCE_DLFE] = DEV_HW_DLFE,
	[IORESOURCE_YUVP] = DEV_HW_YPP,
	[IORESOURCE_SHRP] = DEV_HW_SHRP,
	[IORESOURCE_MCSC] = DEV_HW_MCSC0,
};

static const u32 s2mpu_address_table[SYSMMU_DMAX_S2] = {
	[SYSMMU_CSIS_S0_S2 ... SYSMMU_DMAX_S2 - 1] = 0,
	[SYSMMU_CSIS_S0_S2] = 0x1A1C0054,
	[SYSMMU_D_CSTAT_S2] = 0x1A960054,
	[SYSMMU_BYRP_S0_S2] = 0x1B850054,
	[SYSMMU_RGBP_S0_S2] = 0x1B050054,
	[SYSMMU_MCFP_S0_S2] = 0x1F890054,
	[SYSMMU_YUVP_S0_S2] = 0x1C090054,
	[SYSMMU_LME_S0_S2] = 0x288C0054,
	[SYSMMU_DLNE_S0_S2] = 0x28080054,
};
...
/* SHRP */
static irqreturn_t __is_isr1_shrp(int irq, void *data)
{
	__is_isr_host(data, INTR_HWIP1);
	return IRQ_HANDLED;
}

static irqreturn_t __is_isr2_shrp(int irq, void *data)
{
	__is_isr_host(data, INTR_HWIP2);
	return IRQ_HANDLED;
}
...
/* DLFE */
static irqreturn_t __is_isr1_dlfe(int irq, void *data)
{
	__is_isr_host(data, INTR_HWIP1);
	return IRQ_HANDLED;
}

static irqreturn_t __is_isr2_dlfe(int irq, void *data)
{
	__is_isr_host(data, INTR_HWIP2);
	return IRQ_HANDLED;
}
...
int is_get_hw_list(int group_id, int *hw_list)
{
	...
	case GROUP_ID_YUVP:
		...
		break;
	case GROUP_ID_SHRP:
		hw_list[hw_index] = DEV_HW_SHRP; hw_index++;
		break;
	case GROUP_ID_MCS0:
	...
	case GROUP_ID_MCFP:
		...
		hw_list[hw_index] = DEV_HW_DLFE; hw_index++;
		break;
	case GROUP_ID_MAX:
		break;
	default:
...
int is_hw_get_address(void *itfc_data, void *pdev_data, int hw_id)
{
	...
	case DEV_HW_MCFP:
		...
		break;
	case DEV_HW_DLFE:
		__is_hw_get_address(pdev, itf_hwip, hw_id, "DLFE", IORESOURCE_DLFE, REG_SETA, true);
		break;
	case DEV_HW_YPP:
		__is_hw_get_address(pdev, itf_hwip, hw_id, "YUVP", IORESOURCE_YUVP, REG_SETA, true);
		break;
	case DEV_HW_SHRP:
		__is_hw_get_address(pdev, itf_hwip, hw_id, "SHRP", IORESOURCE_SHRP, REG_SETA, true);
		break;
	case DEV_HW_MCSC0:
...
int is_hw_get_irq(void *itfc_data, void *pdev_data, int hw_id)
{
	...
	case DEV_HW_DLFE:
		...
		if (itf_hwip->irq[INTR_HWIP1] < 0) {
			err("Failed to get irq dlfe-0\n");
			return -EINVAL;
		}

		itf_hwip->irq[INTR_HWIP2] = platform_get_irq(pdev, 16);
		if (itf_hwip->irq[INTR_HWIP2] < 0) {
			err("Failed to get irq dlfe-1\n");
			return -EINVAL;
		}
		break;
	case DEV_HW_YPP:
	...
	case DEV_HW_SHRP:
		...
		if (itf_hwip->irq[INTR_HWIP1] < 0) {
			err("Failed to get irq shrp-0");
			return -EINVAL;
		}
		itf_hwip->irq[INTR_HWIP2] = platform_get_irq(pdev, 20);
		if (itf_hwip->irq[INTR_HWIP2] < 0) {
			err("Failed to get irq shrp-1");
			return -EINVAL;
		}
		break;
	case DEV_HW_MCSC0:
...
int is_hw_request_irq(void *itfc_data, int hw_id)
{
	...
	case DEV_HW_YPP:
		...
		break;
	case DEV_HW_SHRP:
		ret = __is_hw_request_irq(itf_hwip, "shrp-0", INTR_HWIP1,
				IRQF_TRIGGER_NONE, __is_isr1_shrp);
		ret = __is_hw_request_irq(itf_hwip, "shrp-1", INTR_HWIP2,
				IRQF_TRIGGER_NONE, __is_isr2_shrp);
		break;
	case DEV_HW_MCFP:
		...
		break;
	case DEV_HW_DLFE:
		ret = __is_hw_request_irq(itf_hwip, "dlfe-0", INTR_HWIP1,
				IRQF_TRIGGER_NONE, __is_isr1_dlfe);
		ret = __is_hw_request_irq(itf_hwip, "dlfe-1", INTR_HWIP2,
				IRQF_TRIGGER_NONE, __is_isr2_dlfe);
		break;
	case DEV_HW_MCSC0:
...
static void is_hw_set_csis_clk_cb(u32 clock)
{
	info("[MIF(%lu)][INT(%lu)][INTCAM(%lu)][CAM(%lu)][CSIS(%lu)][ISP(%lu)]\n",
		pablo_devfreq_g_domain_freq(DEVFREQ_MIF), pablo_devfreq_g_domain_freq(DEVFREQ_INT),
		pablo_devfreq_g_domain_freq(DEVFREQ_INTCAM),
		pablo_devfreq_g_domain_freq(DEVFREQ_CAM), pablo_devfreq_g_domain_freq(DEVFREQ_CSIS),
		pablo_devfreq_g_domain_freq(DEVFREQ_ISP));
}
...
void is_hw_camif_init(void)
{
	...
	pablo_register_change_dvfs_cb(IS_DVFS_CSIS, is_hw_set_csis_clk_cb);
}
...
void is_hw_csi_qchannel_enable_all(bool enable)
{
	phys_addr_t csis_cmn_ctrls[NUM_OF_CSIS] = {
		0x1A030004, 0x1A040004, 0x1A050004, 0x1A060004,
		0x1A070004, 0x1A080004, 0x1A090004,
	};
	phys_addr_t csis_pdp_top = 0x1A190000;
	void __iomem *base;
	u32 val;
	int i;
...
int is_hw_get_capture_slot(struct is_frame *frame, dma_addr_t **taddr, u64 **taddr_k, u32 vid)
{
	...
	case IS_LVN_CSTAT3_DRC:
		...
		break;
	case IS_LVN_CSTAT0_SAT:
	case IS_LVN_CSTAT1_SAT:
	case IS_LVN_CSTAT2_SAT:
	case IS_LVN_CSTAT3_SAT:
		p = frame->dva_cstat_sat[frame->cstat_ctx];
		break;
	case IS_LVN_CSTAT0_CAV:
	case IS_LVN_CSTAT1_CAV:
	case IS_LVN_CSTAT2_CAV:
	case IS_LVN_CSTAT3_CAV:
		p = frame->dva_cstat_cav[frame->cstat_ctx];
		break;
	case IS_LVN_CSTAT0_PRE_THUMB:
	case IS_LVN_CSTAT1_PRE_THUMB:
	case IS_LVN_CSTAT2_PRE_THUMB:
	case IS_LVN_CSTAT3_PRE_THUMB:
	...
	case IS_LVN_BYRP_HDR:
		...
		break;
	case IS_LVN_BYRP_HDR1:
		p = frame->dva_byrp_hdr1;
		break;
	case IS_LVN_BYRP_RGBY_HIST:
		p = frame->dva_byrp_rgby_hist;
		break;
	case IS_LVN_BYRP_THSTAT_AWB:
		p = frame->dva_byrp_thstat_awb;
		break;
	/* RGBP */
	case IS_LVN_RGBP_HF:
		p = frame->dva_rgbp_hf;
	...
	case IS_LVN_RGBP_RGB:
		...
		break;
	case IS_LVN_RGBP_IN_RGB:
		p = frame->dva_rgbp_in_rgb;
		break;
	...
	case IS_LVN_YUVP_RTA_INFO:
		...
		break;
	case IS_LVN_SHRP_RTA_INFO:
		kp = frame->kva_shrp_rta_info;
		break;
