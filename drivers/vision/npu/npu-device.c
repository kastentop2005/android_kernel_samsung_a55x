...
static int npu_device_probe(struct platform_device *pdev)
{
	...
	ret = npu_dvfs_token_work_probe(device);
	if (ret) {
		probe_err("npu_dvfs_token_work_probe is fail(%d)\n", ret);
		goto err_exit;
	}
	...
	device_initialize(&device->ncp_sync_dev);

#if IS_ENABLED(CONFIG_SOC_S5E9945)
	if ((exynos_soc_info.main_rev == 1) && (exynos_soc_info.sub_rev < 2))
		atomic_set(&device->power_active, NPU_DEVICE_PWR_DEACTIVE);
#endif
...
#if IS_ENABLED(CONFIG_SOC_S5E9945)
	if ((exynos_soc_info.main_rev == 1) && (exynos_soc_info.sub_rev < 2))
		atomic_set(&device->power_active, NPU_DEVICE_PWR_ACTIVE);
#endif
...
static int __init npu_device_init(void)
{
	...
	ret = unpu_register();
	if (ret) {
		probe_err("error(%d) in unpu_register\n", ret);
		goto err_exit;
	}
#endif
